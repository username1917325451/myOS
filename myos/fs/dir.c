...
/* 在分区part上打开编号为inode_no的目录并返回目录指针 */
...
/* 从分区part目录pdir的inode中删去编号为inode_no的目录项 */
bool delete_dir_entry(struct partition *part, struct dir *pdir, uint32_t inode_no, void *io_buf)
{
    struct inode *dir_inode = pdir->inode;
    uint32_t block_idx = 0, all_blocks[MAX_FILE_SIZE] = {0};
    /* 把块地址全部读进all_blocks */
    while (block_idx < 12)
    {
    ...
    /* 目录下可能会有空缺不连续,因此要全部遍历 */
    block_idx = 0;
    while (block_idx < MAX_FILE_SIZE)
    {
        ...
        if (dir_entry_cnt == 1 && !is_dir_first_block)
        {
            ...
            { // 在一级间接索引表中擦除该间接块地址
                ...
                while (indirect_block_idx < MAX_FILE_SIZE)
                {
...
/* 将目录项p_de写入父目录parent_dir的inode中,io_buf是主函数提供的暂存区 */
    ...
    { // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
        ...
        { // 在三种情况下分配块
            ...
            { // 若是间接块未分配
                ...
                // 将第(block_idx-12)个间接块地址更新到一级间接块表(每次会更新sec_cnt*512字节)
                ide_write(cur_part->my_disk, dir_inode->i_sectors[12], all_blocks + 12, 1);
            }
        ...
        /* 若第block_idx块已存在,将其读进内存,放到io_buf中,然后在该块中查找空目录项 */
        ...
        while (dir_entry_idx < dir_entrys_per_sec)
        {
            /*
                存在的问题:
                有一个旧磁盘被格式化了,可能会导致当前的f_type不为0(FT_UNKNOWN)
            */
            if ((dir_e + dir_entry_idx)->f_type == FT_UNKNOWN)
...
struct dir_entry *dir_read(struct dir *dir)
{
    ...
    while (block_idx < block_cnt)
    {
        ...
        {
            // 是否存在对陌生文件的识别问题 ??
            if ((dir_e + dir_entry_idx)->f_type == FT_REGULAR || 
            (dir_e + dir_entry_idx)->f_type == FT_DIRECTORY)
            {
...
/* 从parent_dir的inode中删去child_dir,并释放掉child_dir的inode */
int32_t dir_remove(struct dir *parent_dir, struct dir *child_dir)
{
    struct inode *child_dir_inode = child_dir->inode;
    /* 空目录只在inode->i_sectors[0]中有扇区,其它扇区都应该为空 */
    // 这里应该改为判断child_dir->inode->i_size == cur_part->sb->dir_entry_size * 2 ??
    int32_t block_idx = 1;
    while (block_idx < 13)
    {
        if(child_dir_inode->i_sectors[block_idx] != 0)
        {
            printk("dir_remove: the child_dir is not empty\n");
            return -1;
        }
        block_idx++;
    }
