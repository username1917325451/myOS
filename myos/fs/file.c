...
/* 从全局文件表file_table中获取一个空闲位,成功返回下标,失败返回-1 */
...
/* 创建并打开文件,将目录项安装到父目录的inode下,若成功则返回文件描述符,否则返回-1 */
...
{
    ASSERT(file != NULL && file->fd_inode != NULL);
    if ((file->fd_inode->i_size + count) > (BLOCK_SIZE * MAX_FILE_SIZE))
    {
    ...
    uint32_t size_left = count;    // 用来记录待写入数据大小
    ...
    /*
        file_will_use_blocks是否应该用UPPER_ROUND代替 ??
        :true  错误情况: i_size = 510  count = 2
        这里表示的含义是基于已经使用的块,下一次存储操作会发生在第几个块上
    */
    uint32_t file_has_used_blocks = file->fd_inode->i_size / BLOCK_SIZE + 1;

    /* 存储count字节后该文件将占用的块数 */
    uint32_t file_will_use_blocks = (file->fd_inode->i_size + count) / BLOCK_SIZE + 1;
    ASSERT(file_will_use_blocks <= MAX_FILE_SIZE);
    ...
    */
    ...
    { // 直到写完所有数据
        ...
        sec_idx = file->fd_inode->i_size / BLOCK_SIZE; // 下一个待写入的块的编号
        sec_lba = all_blocks[sec_idx];
        sec_off_bytes = file->fd_inode->i_size % BLOCK_SIZE;
        sec_left_bytes = BLOCK_SIZE - sec_off_bytes; // 当前块剩下的字节数
