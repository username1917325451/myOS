...
// 传入inode的索引, 获取inode所在的扇区和扇区内的偏移量
static void inode_locate(struct partition *part, uint32_t inode_no, struct inode_position *inode_pos)
{
    ...
    { // 若扇区内剩下的空间不足以容纳一个inode,必然是inode跨越了当前扇区和下一个扇区
...
{ // io_buf是用于硬盘io的缓冲区
    ...
    inode_locate(part, inode_no, &inode_pos); // 获取inode在磁盘中的位置信息
...
// 根据inode编号加载对应inode到内存中,并返回inode
...
/* 从分区part中删去编号为inode_no的inode数据结构 */
...
/* 回收分配给inode的数据块并调用inode_delet释放掉inode本身 */
