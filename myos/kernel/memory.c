...
void block_desc_init(struct mem_block_desc* desc_array) {				   
   	...
   	for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++) {
      	...
      	block_size *= 2;         // 更新为下一个规格内存块 最大为1024字节
   }
}
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    // 找到这个虚拟地址对应的目录表中的索引    *pde的值即为vaddr的二级页表的物理地址
   	uint32_t* pde = pde_ptr(vaddr);
    // 找到这个虚拟地址对应的二级页表中的索引  *pte的值即为vaddr真实分配到的物理地址
 ...
 * *********************************************************/
   /* 先在页目录内判断目录项的P位，若为1,则表示该表已存在 */
   	if (*pde & 0x00000001) {	 // 页目录项和页表项的第0位为P,此处判断目录项是否存在
    	...
    	if (!(*pte & 0x00000001)) {
			// 将vaddr在二级页表中的对应索引指向page_phyaddr这片内存
...
// 清除_vaddr开始的pg_cnt个虚拟页表的bitmap标志位
		...
		if (list_empty(&descs[desc_idx].free_list)) {
			...
			* cnt置为此arena可用的内存块数,large置为false */
			...
			for (block_idx = 0; block_idx < descs[desc_idx].blocks_per_arena; block_idx++) {
				...
				// 并没有真实地创建 mem_block, 而是将分配到的内存块的以mem_block的形式插入进free_list
				// 每一块block_size大小的块的起始都是一个mem_block
