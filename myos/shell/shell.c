...
/* 将cmd_str以空格切割,并将切割后的单词存入argv数组,返回单词数 */
static int32_t cmd_parse(char *cmd_str, char **argv, char token)
{
    ...
    char *next = cmd_line_for_argv;
    strcpy(cmd_line_for_argv, cmd_str);
    int32_t argc = 0;
    /* 外层循环处理整个命令行 */
    while (*next)
    {
...
void cmd_execute(uint32_t argc, char **argv)
{
    ...
    { // 如果是外部命令,需要从磁盘上加载
        ...
        { // 父进程
            ...
            // printf会往标准输出里写数据,此时的标准输出可能被重定向
