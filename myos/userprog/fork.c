...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct *child_thread, struct task_struct *parent_thread)
{
    ...
    // 之前被分配的物理页属于父进程不与子进程共享,因此block也要重新初始化
        ...
        if (vaddr_btmp[idx_byte])
        {
            ...
            while (idx_bit < 8)
            {
                if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte])
                {
                    ...
                    /* c 申请虚拟地址prog_vaddr */
                    /*
                        为什么不用修改子进程的vaddr_bitmap ??
                        : 因为prog_vaddr早已在父进程的vaddr_bitmap中记录,
                          而子进程的vaddr_bitmap直接复制于父进程,只是子进
                          程的页表中没有相应映射,因此补充映射即可
                    */
...
static int32_t build_child_stack(struct task_struct *child_thread)
{
    ...
    /* 
        调用中断返回时,中断栈中的gs,fs等寄存器数据从哪里取出来 ??
        父进程调用fork时会进入中断,
        由于子进程的intr_stack中拷贝了父进程进入中断时的用户栈信息,cs: ip 信息,
        所以中断退出后,子进程将会继续执行父进程之后的代码。
    */
